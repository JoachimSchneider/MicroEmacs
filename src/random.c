	...
	if ((i = lookup_color(cbuf)) != -1)
		...
		if (global)
			...
			if (uflag)
				{
				gfcolor = i;
				TRC(("Globally setting foreground color to %s/%d", (char *)cbuf, (int)i));
				}
			else
				{
				gbcolor = i;
				TRC(("Globally setting background color to %s/%d", (char *)cbuf, (int)i));
				}
#if	WINDOW_TEXT & 0
			...
			if (uflag)
				{
				curwp->w_fcolor = i;
				TRC(("Locally setting foreground color to %s/%d", (char *)cbuf, (int)i));
				}
			else
				{
				curwp->w_bcolor = i;
				TRC(("Locally setting background color to %s/%d", (char *)cbuf, (int)i));
				}
