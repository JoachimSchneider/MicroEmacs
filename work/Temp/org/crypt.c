 ...
 *       the starting key affects the entire message. We go through
 *       some effort here to prevent the key from exceeding 29 bits.
 *       This is so the aritmetic caluclation in a later statement,
 *       which impliments our autokey, won't overflow, making the key
 *       go negative. Machine behavior in these cases does not tend to
 *       be portable.
 **/
