...
#if     COLOR
/* palette table. It is initialized for the default colors, but can be changed
 * by the spal function */
...
VOID FAR PASCAL BuildCellMetrics (CellMetrics *cm, HFONT hFont)
{
    ...
    if ( (cm->OffsetY = (cm->SizeY / 8) - cm->HalfLeadingY) < 0 ) {
        cm->OffsetY = 0;
        /* for a reasonable upper boundary separation, we want (SizeY / 8) <=
         * OffsetY + HalfLeadingY */
    }
                                 ...
                                 int  bottomrow)
{
    ...
    POINT MaxBottomRight;   /* Cell coordinates of bottom right of MDI window */
    ...
    /* this adjustment is done to avoid left-over pixels caused by characters
     * which display an overhang outside their cells (for instance: w & W in
     * Courier New). The corresponding adjustment is done on Rect.left a few
     * lines above */
    if ( bottomrow + 1 >= MaxBottomRight.y ) Rect.bottom = ClientRect.bottom;

    InvalidateRect (hWnd, &Rect, FALSE);
    /* the background does not need erasing since Paint will completely fill the
     * invalid rectangle */
} /* InvalidateCells */
...
VOID FAR PASCAL MinimumClientSize (HWND hWnd,
                                   ...
                                   int  *Width,
                                   int  *Height)
/* The values pointed by Height and Width are set to the smallest value that
 * allows displaying NRows rows and NCols columns. A NULL pointer disables the
 * computing of the associated value */
{
...
/* Heigh is the hypothetic heigh of the client area. If this parameter is 0, the
 * real client area is measured. If it is negative, the maximal client area
 * (maximized child in a maximized frame) is used.
 */
{
    RECT Rect;
    int x;
...
/* DisplayableColumns:  returns the number of columns displayable in the client
 * area */
/* ==================
 *                                                                */

int FAR PASCAL DisplayableColumns (HWND hWnd, int Width, CellMetrics *cm)
/* Width is the hypothetic width of the client area. If this parameter is 0, the
 * real client area is measured. If it is negative, the maximal client area
 * (maximized child in a maximized frame) is used.
 */
{
    ...
    if ( Show ) {
        ...
        if ( hFrameWnd == GetActiveWindow () ) {
            if ( !IsWindow (hCaretWnd) ) {
                /* this may happen in some transient cases when closing down a
                 * screen */
                hCaretWnd = 0;
 ...
 * HalfLeadingY-high areas above and under the cell */
{
    ...
    if ( (Cell->x = (Client.x - EmacsCM.OffsetX) /EmacsCM.SizeX) <
         0 ) Cell->x = 0;
...
VOID FAR PASCAL GetMinMaxInfo (HWND hWnd, LPPOINT rgpt)
{
    ...
    if ( notquiescent ) {
        ...
        rgpt[3].y = Y + GetSystemMetrics (SM_CYCAPTION) +
                    ( 2 * GetSystemMetrics (SM_CYFRAME) ) +GetSystemMetrics (
            SM_CYHSCROLL);
    }
} /* GetMinMaxInfo */
...
VOID FAR PASCAL ScrPaint (HWND hWnd)
{
    ...
    /* note that if the WM_PAINT is not passed through the message loop (as
     * would hapen with use of UpdateWindow), it is possible to attempt
     * repainting while defferupdate is TRUE which means update has been
     * deffered */
    ...
    CellToClient (hWnd, *(POINT *)&Rect.left, (POINT *)&Rect.left);
    CellToClient (hWnd, *(POINT *)&Rect.right, (POINT *)&Rect.right);
    /* Rect now contains a bounding rectangle for the 1st row. This will be used
     * to paint the row's background and will be moved down one row's height at
     * each iteration of the painting loop */
        ...
        if ( !(vp->v_flag & VFNEW) ) {    /* valid contents */
            ...
            * boundaries (reverse video is applied between [1] and [2] */
            for ( i = 0; i < 3; i++ ) if ( Boundary[i] < Boundary [i+1] ) {
                    ...
                    if ( ColorDisplay ) {
                        FColor =
                            EmacsPalette[i == 1 ?vp->v_bcolor : vp->v_fcolor];
                        BColor =
                            EmacsPalette[i == 1 ?vp->v_fcolor : vp->v_bcolor];
