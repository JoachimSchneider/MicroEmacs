...
COLOR xvt_colors[] =    /* xvt color translation table */
{
    ...
    COLOR_MAGENTA, COLOR_CYAN, COLOR_WHITE, COLOR_BLACK, COLOR_RED, COLOR_GREEN,
...
/*          sgarbf = TRUE;*/
    ...
    case E_MOUSE_UP:                    /* a button has been released */
        ...
        if ( (ep->type == E_MOUSE_MOVE) &&(mouserow == oldrow) &&
             (mousecol == oldcol) )
            return;
...
in_fkey(xvt_code)       /* input an xvt key code into the MicroEMACS input
                         * stream */
...
/* dump_event("tsk", ep); */
    ...
    case E_CREATE:
        ...
        xstrcat(aname, " ");
        xstrcat(aname, VERSION);

        /* create the first screen window */
        xvt_win = create_window(W_DOC,
                                &rct,
                                aname,
                                0,
                                SCREEN_WIN,
                                WSF_SIZE|WSF_CLOSE|/*WSF_HSCROLL|
