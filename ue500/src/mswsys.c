...
static BOOL SystemModal = FALSE;    /* indicates that the Yes/No message box
                                     * should be system-modal */
...
/* when a long operation is signaled at least twice, the hourglass cursor comes
 * up */
/* While the long operation is in progress, this function runs the message loop
 * approximately every 100 ms to yield to other applications and allow a reduced
 * set of commands (among which quit) to be input by the user */
{
    ...
    if ( f ) {
        if ( !LongOperation ) {
            ...
            /* 2nd (or more) call in a row, let's yield if enough time has
             * elapsed */
            DWORD Time;
...
/* This function replaces the mlyesno from input.c. Instead of asking a question
 * on the message line, it pops up a message box */
{
    if ( MessageBox ( hFrameWnd, prompt, ProgName,
                      MB_YESNO | MB_DEFBUTTON2 | MB_ICONQUESTION |
                      (SystemModal ? MB_SYSTEMMODAL : 0) ) ==
         IDYES ) return TRUE;
...
/* returns FALSE if failed init */
{
    ...
    GetPrivateProfileString (ProgName, "InitialSize", "", text, TXTSIZ,
                             IniFile);
    strlwr (text);
    ...
    GetPrivateProfileString (ProgName, "CaretShape", "", text, TXTSIZ, IniFile);
    strlwr (text);
    ...
    hNotQuiescentCursor = LoadCursor (hEmacsInstance, "NotQuiescentCursor");
#if GRINDERS == 0
...
/* If MsgParam is not zero, the broadcast is sent as an EmacsBroadcastMsg to all
 * the Emacs frame windows, except the one specified by hFrameWnd. The wParam of
 * that message is hFrameWnd, lParam is set to MsgParam. The value returned is
 * the highest returned value from the broadcast or zero if no other emacs
 * instance was found.
 *
 ...
 */
{
    ...
    ProcInstance =
        MakeProcInstance ( (FARPROC)BroadcastEnumProc, hEmacsInstance );
    BroadcastVal = 0;
...
LONG EXPORT FAR PASCAL MDIClientSubProc (HWND   hWnd,
                                         UINT   wMsg,
                                         WPARAM wParam,
                                         LPARAM lParam)
{
    ...
    case WM_PARENTNOTIFY:
        if ( notquiescent ) break;  /* blocks mouse selection of an MDI child
                                     * (except on caption clicks!)
                                     */
        goto DefaultProc;

    default:
DefaultProc: return CallWindowProc (MDIClientProc, hWnd, wMsg, wParam, lParam);
    }
...
VOID FAR PASCAL FrameInit (CREATESTRUCT *cs)
{
    ...
    /*-advertise our birth to other Emacs instances and set the frame's caption
     * with an Id if necessary */
    if ( ( FrameId =
               (UINT)EmacsBroadcast ( MAKELONG(EMACS_STARTING,
                                               EMACS_PROCESS) ) ) != 0 ) {
        FrameId++;
    }
    ...
    hMDIClientWnd = NULL;   /* the NULL initial value allows DefFrameProc to
                             * behave properly if FrameInit fails */
...
/* returns TRUE if emacs should exit */
{
    ...
    if ( fbusy != FWRITING ) {   /* no file write in progress */
        ...
        /* there is, indeed a file write in progress MessageBeep (0);    /* wake
         * the user up! */
    ...
    update (TRUE);          /* restore the caret that quit() moved into the
                             * message line */
                                   ...
                                   LPARAM lParam)
{
    ...
    case WM_CREATE:
        ...
        {   /*-setup the stuff for display.c */
            ...
            sp =
                (SCREEN_T*)( ( (MDICREATESTRUCT*)( ( (CREATESTRUCT*)lParam )->
                                                   lpCreateParams ) )
                             ->lParam );
#if WINXP
        ...
        if ( wParam ) {
            ...
            if ( !InternalRequest ) {
...
#if WINXP
                select_screen( (SCREEN_T*)GetWindowLongPtr(hWnd,
                                                           GWL_SCRPTR), FALSE );
#else
                select_screen( (SCREEN_T*)GetWindowLong(hWnd, GWL_SCRPTR),
                               FALSE );
#endif
                ...
                if ( sp->s_virtual == NULL ) {
                    ...
                    vtinitscr ( sp, DisplayableRows (hWnd, 0,
                                                     &EmacsCM), DisplayableColumns (
                                    hWnd, 0,
                                    &EmacsCM) );
                }
            }
    ...
    case WM_MOUSEACTIVATE:
        if ( notquiescent ) break;  /* blocks mouse selection of an MDI child's
                                     * caption */
        goto DefaultProc;
    ...
    case WM_SYSCOMMAND:
        ...
        case SC_MOVE:
            if ( notquiescent ) {
                /* prevent moving an MDI client other than the already active
                 * one, since it would transfer the activation */
                if ( hWnd !=
                     (HWND)SendMessage (hMDIClientWnd, WM_MDIGETACTIVE, 0,
                                        0L) ) break;
            }
            goto DefaultProc;
    ...
    default:
DefaultProc: return DefMDIChildProc (hWnd, wMsg, wParam, lParam);
    }
                                     ...
                                     LPARAM lParam)
{
    ...
    case WM_SIZE:   /* must adjust the MDIClient's size to fit the Message Line
                     */
        MoveWindow (hMDIClientWnd,
                    0,
                    0,
                    LOWORD(lParam),
                    HIWORD(lParam) - EmacsCM.MLHeight,
                    TRUE);
    ...
    case WM_NCLBUTTONDBLCLK:
        if ( Win31API ) goto DefaultProc;
            ...
            /* This corrects a Windows 3.0 bug that prevents a maximized child
             * from being closed by double-clicking the close-box. See "Windows
             * 3: A Developer's Guide" by Jeffrey M. Richter, page 480 */
            RECT Rect;
            HBITMAP hBmp;
            BITMAP Bmp;
            ...
            Rect.top += GetSystemMetrics(SM_CYCAPTION) +GetSystemMetrics (
                SM_CYFRAME);
            ...
            SendMessage(LOWORD(MDIGetActivateResult),
                        WM_SYSCOMMAND,
                        SC_CLOSE,
                        lParam);
        }
    ...
    case WM_ACTIVATE:
        ...
        /* This call matters only when the active MDI Child is iconic, in which
         * case it never receives the focus and thus fails to create the caret
         * (that should still appear if we are getting input on the message
         * line) */
        goto DefaultProc;
    ...
    case WM_DESTROY:
#if WINXP
        if ( MainHelpUsed ) HtmlHelp (hFrameWnd,
                                      MainHelpFile,
                                      HH_CLOSE_ALL,
                                      NULL);
        if ( HelpEngineFile[0] != '\0' ) HtmlHelp (hFrameWnd,
                                                   HelpEngineFile,
                                                   HH_CLOSE_ALL,
                                                   NULL);
#else
        if ( MainHelpUsed ) WinHelp(hFrameWnd, MainHelpFile, HELP_QUIT, NULL);
        if ( HelpEngineFile[0] != '\0' ) WinHelp(hFrameWnd,
                                                 HelpEngineFile,
                                                 HELP_QUIT,
                                                 NULL);
#endif
    ...
    default:
        if ( (wMsg == EmacsBroadcastMsg) &&(HIWORD(lParam) == EMACS_PROCESS) ) {
            UINT Id;

            switch ( LOWORD(lParam) ) {
            case EMACS_STARTING:
                /* another instance of emacs is starting. If our ID was 0, it
                 * should now be 1 */
...
DefaultProc: return DefFrameProc (hWnd, hMDIClientWnd, wMsg, wParam, lParam);
    }
                     ...
                     int       nCmdShow)
{
    ...
    /* If we exit through an emacs command, we pass here. Otherwise (exit
     * through a close in system menu or a windows session termination), the
     * application terminates from the call to mswgetc that received the signal
     */
    case ABORT:
        eexitflag = TRUE;
        longop (FALSE);
 ...
 * emptied into the in_put pipe. If WaitMode is FALSE, this function uses
 * PeekMessage from the beginning and returns only when the input queue is
 * empty. In that case, WM_PAINTs are not processed, but the processor still
 * gets relinquished. Note that while peeking for messages, this function
...
/* this function is used by mswsleep(). It returns TRUE unless the timer could
 * not be created. Note that for a null time, it simply relinquishes the
 * processor */
{
    TakingANap = TRUE;
    if ( (t == 0) ||( !SetTimer (hFrameWnd, T_SLEEP, t, (TIMERPROC)NULL) ) ) {
        ShowEmacsCaret (TRUE);
...
static BOOL PASCAL UpdateCursor (HWND hWnd, WPARAM wParam, LPARAM lParam)
/* this function should be called on each WM_SETCURSOR message, to display the
 * appropriate cursor. It returns TRUE if all processing has been done (the
 * calling window function should return TRUE). it returns FALSE if the default
 * window proc sould be called. */
{
    HCURSOR hCursor;
...
/* hg = TRUE sets the hourglass, hg = FALSE removes it */
{
    ...
    if ( hWnd && ( (hWnd == hFrameWnd) || IsChild (hFrameWnd, hWnd) ) ) {
        ...
        /* this should cause a dummy cursor movement to be processed, ending up
         * in UpdateCursor being invoked */
    }
} /* SetHourglass */
