...
static HANDLE hClipData = NULL;     /* used by insertclip and ClipboardCleanup
                                     */
...
NoClipboardMemory: mlabort (TEXT94);   /* out of memory */
        Result = FALSE;
    }
...
int PASCAL insertclip (int f, int n)
{
    ...
    if ( OpenClipboard (hFrameWnd) ) {
        if ( ( hClipData = GetClipboardData (CF_TEXT) ) != NULL ) {
            ...
            if ( yankflag ) {
                /* Find the *previous* line, since the line we are on may
                 * disappear due to re-allocation.  This works even if we are on
...
bail_out:       GlobalUnlock (hClipData);
                hClipData = NULL;   /* for ClipboardCleanup */
                /* If requested, set global "." back to the beginning of the
                 * yanked text. */
                if ( yankflag ) {
...
int PASCAL helpengine (int f, int n)
{
    ...
    SetWorkingDir ();
    if ( ( Result =
               FILENAMEREPLY (TEXT307, HelpEngineFile,
                              NFILEN) ) != TRUE ) return Result;
...
{
    ForceMessage (hMDIClientWnd,
                  WM_MDIMAXIMIZE,
                  (UINT)first_screen->s_drvhandle,
                  0L);
...
/* without a numeric argument, tile horizontally. With a numeric argument of 1,
 * tile vertically */
{
    if ( f && (n == 1) ) {
...
void FAR PASCAL ScrollMessage (HWND hWnd, UINT wMsg, WORD ScrlCode, int Pos)
{
    ...
    if ( wMsg == WM_VSCROLL ) {
        ...
        case SB_THUMBPOSITION:
            if ( Win31API ) {
                ...
                /* there is something wrong with thumb tracking in Windows 3.0
                 */
            }
    ...
    /* this ensures we go through the editloop(), updating the modeline display
     * and running the cmdhook, among other things */
...
/* the w_flag is used to determine what needs updating: if the WFHARD bit is
 * set, both scroll bars need an update. If the WFMOVE bit is set, the
 * horizontal scroll bar needs an update */
/* this function assumes s_cur_window matches curwp for the first_screen */
{
    ...
    if ( vscrollbar && (w_flag & WFHARD) ) {
        ...
        GetScrollRange ( (HWND)sp->s_drvhandle, SB_VERT, &ScrollMin,
                         &ScrollMax );
        if ( (ScrollMax != lastline) || (ScrollMin != 1) ) {
            SetScrollRange ( (HWND)sp->s_drvhandle, SB_VERT, 1, lastline,
                             FALSE );
        ...
        if ( topline != ScrollPos ) {
            SetScrollPos ( (HWND)sp->s_drvhandle, SB_VERT, topline, TRUE );
        }
    }
    if ( hscrollbar && ( w_flag & (WFMOVE | WFHARD) ) ) {
        ...
        GetScrollRange ( (HWND)sp->s_drvhandle, SB_HORZ, &ScrollMin,
                         &ScrollMax );
        if ( (ScrollMax != maxlength) || (ScrollMin != 0) ) {
            SetScrollRange ( (HWND)sp->s_drvhandle, SB_HORZ, 0, maxlength,
                             FALSE );
            ScrollPos = -1; /* makes sure the scroll display is updated */
        } else ScrollPos = GetScrollPos ( (HWND)sp->s_drvhandle, SB_HORZ );
        if ( row != ScrollPos ) {
            SetScrollPos ( (HWND)sp->s_drvhandle, SB_HORZ, row, TRUE );
        }
    }
} /* updscrollbars */
