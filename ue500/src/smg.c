 ...
 *
 *  Consults the system's terminal tables for both DEC terminals, foreign and
 * user defined terminals.
 *
 *  Author:  Curtis Smith Update history:
 *  14-Jul-1987, first revision.
 *  02-Jan-1988, by J. A. Lomicka: Code addition for interpreting LK201 function
 * keys, application keypad and cursor position reports.
 *  09-Apr-1988, second revision. Major changes:
 *      1) H files removed; replaced with globalvalues.
 *      2) Terminal now left in ECHO mode. Read call disables echo.
 *      3) TYPAHD macro now performs correctly.
 *      4) $sres variable now accepts NORMAL and WIDE.
 *      5) Writes to screen now use QIO without waiting. This gives a slight
 * increase in performance. Had to make some QIOW a different event flag number.
 *      6) Function keys, special keys and arrow keys are now bound using the
 * following table:
 *          FNN-DOWN            FNS-E1 FNB-LEFT         FNC-E2 FNF-RIGHT            FND-E3
 * FNP-UP               FN@-E4 FN^1-PF1         FNZ-E5 FN^2-PF2         FNV-E6
 * FN^3-PF3 FN^4-PF4
 *
 *          A-0 thru A-9: Keypad 0 thru 9 A-E: Enter A-., A-, : Keypad . and ,
 *
 *          FN1 thru FN0 : Function keys 1 thru 10 S-FN1 thru S-FN0 : Function
 * keys 11 thru 20
 *
 *      See ebind.h for key bindings.
 *  2-dec-88 Curtis Smith
 *  - These have been rebound to the new machine independant bindings
 * 27-March-1989    J.A.Lomicka Separate VMS.C from SMG.C, This file is now
 * SMG.C, and only contains the SMG stuff.
 *
 *
 *  17-jul-89 : mike ward Modifications for proper operation of special keys.
 * Basically, the changes are to deal with key values as ints rather rather than
 * characters so that when a character sequence is recognized as a special key,
 * the approriate value can be substituted for the sequence directly (in inbuf).
 *
 *      1)  Use "int" rather than "char" to declare inbuf[], inbuft, and *inbufh
 * since keys use more than 8-bits 2)   Change the parameter declaration "char
 * ch" to "int ch"
 *          in smgqin since we queue key values (not characters) 3)     Change
 * calls "smgqin(ch)" to "smgqin((unsigned char)ch)"
 *          in cook since we don't want sign extension 4)   Remove call
 * "smgqin(0)" in cook. Instead of queuing a sequence of chars to represent the
 * key value, just queue the final value instead - much cleaner.
 *      5)  Change the declaration from "char ch" to "int ch" in smggetc since
 * ch now holds a key value rather than a character.
 *  11-dec-89:  Kevin A. Mitchell Don't restore numeric keypad if the user had
 * it set that way in DCL.
 */
...
/*
 *       Empty routine make some compilers happy when SMG is not defined, and is
 * also used as a noop routine in the terminal dispatch table.
 */
smg_noop()
{
}
 ...
 *
 *  smgmove calls to the SMG run-time library to produce a character sequence to
 * position the cursor.  If the sequence cannot be made, a string "OOPS" is
 * produced instead, much like the termcap library under UNIX.  In the case of
 * "OOPS", the user will soon know that his terminal entry is incorrect.
 *
 *  Nothing returned.
 ***/
 ...
 *  smgcres  -  Change screen resolution
 *
 *  smgcres changes the screen resolution of the current window. Allowable sizes
 * are NORMAL and WIDE.
 *
 *  Nothing returned
 ***/
 ...
 *
 *  smgrev either sets or resets the reverse video state, based on the boolean
 * argument.  This function is only called if the revexist boolean variable is
 * set to TRUE.  Otherwise there is no reverse video available.
 *
 *  Nothing returned.
 ***/
smgrev(int status)
 ...
 *  smgeeol  -  Erase to end of line
 *
 *  When this function is called, the lines worth of text after the cursor is
 * erased.  This function is only called if the eolexist boolean variable is set
 * to TRUE.  Otherwise the display manager will produce enough spaces to erase
 * the line.
 *
 *  Nothing returned.
 ***/
 ...
 *
 *  smgeeop really should be called smgclear because it really should be an
 * erase screen function.  When called, this routine will send the erase entire
 * screen sequence to the output.
 *
 *  Nothing returned.
 ***/
smgeeop()
 ...
 *
 *  smgbeep send a bell character to the output.  It might be possible in the
 * future to include the NOISY definition and attempt to flash the screen,
 * perhaps using LIGHT_SCREEN and DARK_SCREEN.
 *
 *  Nothing returned.
 ***/
smgbeep()
 ...
 *
 *  smggetstr attempts to obtain the escape sequence for a particular job from
 * the SMG library.  Most sequences do not require a parameter with the
 * sequence, others do.  In order to obtain the definition without knowing ahead
 * of time whether ornot the definition has a parameter, we call SMG once with a
 * parameter and if that fails, we try again without one.  If both attempts
 * fail, we will return the NULL string.
 *
 *  Storage for the sequence comes from a local pool.
 *
 *  Returns:    Escape sequence NULL    No escape sequence available
 ***/
 ...
 *
 *  smggetnum attempts to get a numerical constant from the SMG package. If the
 * constant cannot be found, -1 is returned.
 *
 *  code - SMG code
 ***/
...
/***
 *  smgaddkey - translate the SMG code to the escape sequence, and add it to the
 * list of keys.
 ***/
 ...
 *
 *  smgcap retrives all the necessary capabilities from the SMG library to
 * operate MicroEMACS.  If an insufficent number of capabilities are found for
 * the particular terminal, an error status is returned.
 *
 ...
 *
 *  smgputs is a short-cut routine to handle sending a string of characters to
 * the character output routine.  A check is made for a NULL string, while is
 * considered valid.  A NULL string will produce no output.
 *
 *  Nothing returned.
 ***/
smgputs(string)
 ...
 *
 *  qin queues the character into the input buffer for later reading.  This
 * routine will mostly be used by mouse support and other escape sequence
 * processing.
 *
 *  Nothing returned.
 *
 ...
 *
 *  smggetc obtains input from the character input queue.  If the queue is
 * empty, a call to cook() is called to fill the input queue.
 *
 *  Returns:    character
 ***/
int smggetc()
 ...
 *
 *  spal sets the palette colors for the 8 colors available.  Currently, there
 * is nothing here, but some DEC terminals, (VT240 and VT340) have a color
 * palette which is available under the graphics modes. Further, a foreign
 * terminal could also change color registers.
 *
 *  Nothing returned
 ***/
spal()
 ...
 *  Returns:    status.
 *
 *  flag - TRUE if default n    - Numerical argument
 ***/
