...
/* just like SelectObject, this function returns the previously selected font
 * handle */
{
    HFONT hPrevFont;
 ...
 * returned. If not NULL, FaceName must point to a string containing at least
 ...
 * yet) */
{
    ...
    lf.lfStrikeOut = 0;
    lf.lfCharSet =
        IsDlgButtonChecked (hDlg, ID_ANSI) ?ANSI_CHARSET : OEM_CHARSET;
...
/* =============                                                    */
int EXPORT FAR PASCAL EnumSizesProc (LPLOGFONT    lf,
                                     LPTEXTMETRIC tm,
                                     short        FontType,
                                     LPSTR        Data)
    ...
    if ( FontType & TRUETYPE_FONTTYPE ) {
        ...
        if ( tm->tmAveCharWidth == Metrics.tmAveCharWidth ) {
            AddSize (*(HWND FAR *)Data,
                     (short int)lf->lfHeight,
                     (short int)lf->lfWidth);
        }
    }
 ...
 * retrievable by LB_GETITEMDATA: width in the low-order word and height in the
 * high order word */
{
    ...
    SendDlgItemMessage (hDlg, ID_FONTSIZE, CB_RESETCONTENT, 0, 0L);
    {
        ...
        hDC = GetDC (hDlg);
        ProcInstance =
            MakeProcInstance ( (FARPROC)EnumSizesProc, hEmacsInstance );
    ...
    {
        /*-Select the larger height that is smaller or equal to the current
         * Metrics (assumed to be the ones of the previous font)
         */
        int i;
        int BestIndex = 0;
        ...
        for ( i = 0;; i++ ) {
            ItemData = SendDlgItemMessage (hDlg,
                                           ID_FONTSIZE,
                                           CB_GETITEMDATA,
                                           i,
                                           0L);
...
static VOID PASCAL AddFace (HWND hDlg, char *CandidateFace)
{
    ...
    CharSet = (IsDlgButtonChecked (hDlg, ID_ANSI) ?ANSI_CHARSET : OEM_CHARSET);
    /*-try to identify a fixed-pitch font of set CharSet, with the specified
     * face name */
    hFixedFont = CreateFont (0,
                             0,
                             0,
                             0,
                             0,
    ...
    if ( (tm.tmPitchAndFamily & 0x01) || /* =1 if variable pitch */
         (tm.tmCharSet != CharSet) ||(strcmp (FaceName, CandidateFace) != 0) ) {
        /* flunked the exam! */
        return;     /* skip that face */
    }
    /* the candidate face has at least one fixed-pitch font with the right
     * charset */
    /*- scan the list box to make sure this face is not a duplicate */
    At = -1;    /* start at beginning of list box */
    do {
        From = At;
        At = SendDlgItemMessage (hDlg,
                                 ID_FONT,
                                 LB_FINDSTRING,
                                 From,
    ...
    } while (At > From);    /* exit if search has gone through the bottom of the
                             * list box */
...
/* =============                                                    */
int EXPORT FAR PASCAL EnumFacesProc (LPLOGFONT    lf,
                                     LPTEXTMETRIC tm,
                                     short        FontType,
                                     LPSTR        Data)
/* Data should point to a handle to the dialog box */
 ...
 * current charset selection and then selects an item */
{
    ...
    SendDlgItemMessage (hDlg, ID_FONT, LB_RESETCONTENT, 0, 0L);
    {
        ...
        hDC = GetDC (hDlg);
        ProcInstance =
            MakeProcInstance ( (FARPROC)EnumFacesProc, hEmacsInstance );
...
/* ===========                                 */
int EXPORT FAR PASCAL  FontDlgProc (HWND   hDlg,
                                    UINT   wMsg,
                                    WPARAM wParam,
                                    LPARAM lParam)
{
    ...
    case WM_INITDIALOG:
        ...
        {
            /*-simulate a mouse click on the appropriate charset radiobutton.
             * This will initialize all the other controls
             */
            WORD id;
...
AcceptFont:     if ( hEmacsFont ) DeleteObject (hEmacsFont);
                hEmacsFont = hNewFont;
                EndDialog (hDlg, TRUE);
                /* no need to unhook the SAMPLE's font, it is not destroyed
                 * since it will be used by emacs */
            } else return FALSE;
        ...
        case ID_SAVEFONT:
            if ( NOTIFICATION_CODE == BN_CLICKED ) {
                /*-save the facename, bold status and size into EMACS.INI, then
                 * perform as the OK button */
                char NumText[17];

                GetFontMetrics (hNewFont, &Metrics, FaceName);

                WritePrivateProfileString (ProgName,
                                           "FontName",
                                           FaceName,
                                           IniFile);
                WritePrivateProfileString (ProgName,
                                           "CharSet",
                                           itoa (Metrics.tmCharSet, NumText,
                                                 10),
                                           IniFile);
                WritePrivateProfileString (ProgName,
                                           "FontHeight",
        ...
        case ID_OEM:
            if ( NOTIFICATION_CODE == BN_CLICKED ) {
                CheckRadioButton ( hDlg, ID_1ST_CHARSET, ID_LAST_CHARSET, LOWORD(
                                       wParam) );
...
CancelFont: EndDialog (hDlg, FALSE);
            if ( hNewFont ) {
...
static VOID PASCAL ChangeFont (void)
{
    ...
    /*-loop through all the screens, resizing the vision that emacs has of them,
     * processing the current ("first") screen last */
    InternalRequest = TRUE;
    fsp = first_screen;
    do {
        sp = first_screen;
        while ( sp->s_next_screen != (SCREEN_T *)NULL ) sp = sp->s_next_screen;
        select_screen (sp, FALSE);
        GetClientRect (sp->s_drvhandle, &Rect);
        newwidth ( TRUE,
                   DisplayableColumns (sp->s_drvhandle, Rect.right, &EmacsCM) );
        newsize ( TRUE,
                  DisplayableRows (sp->s_drvhandle, Rect.bottom, &EmacsCM) );
    } while (sp != fsp);
...
/* returns TRUE is a new font has been picked */
{
    ...
    ProcInstance = MakeProcInstance ( (FARPROC)FontDlgProc, hEmacsInstance );
    FontChanged = DialogBox (hEmacsInstance, "FONTS", hFrameWnd, ProcInstance);
    FreeProcInstance (ProcInstance);
...
VOID FAR PASCAL FontInit (void)
{
    ...
    if ( GetPrivateProfileString (ProgName, "CharSet", "", text, 20,
        ...
        lf.lfWeight =
            GetPrivateProfileInt (ProgName, "FontWeight", 400, IniFile);
