/*  Crypt:  Encryption routines for MicroEMACS written by Dana Hoggatt and
 * Daniel Lawrence
 */
...
*
*   (C) Copyright 1986, Dana L. Hoggatt 1216, Beck Lane, Lafayette, IN
*
...
* public domain text/program editor.
*
*   I kept the following goals in mind when preparing this function:
*
*       1.  All printable characters were to be encrypted back into the
* printable range, control characters and high-bit characters were to remain
* unaffected.  this way, encrypted would still be just as cheap to
...
*       on another computer.
*
*       3.  The encryption had to be inexpensive, both in terms of speed and
* space.
*
*       4.  The system needed to be secure against all but the most determined
* of attackers.
*
...
*   to specify an encryption key.  This key is an arbitrary string, to be
* supplied by the user.  To set the key takes two calls to
...
*   this purpose, the best results will be obtained by avoiding multiples of 95.
*
...
*   must remain out of sight.  This system satisfies this set of conditions to
* within the degree desired for MicroEMACS.
*
*   Though direct methods of attack (against systems such as
*   this) do exist, they are not well known and will consume
*   considerable amounts of computing time.  An exhaustive search requires over
* a billion investigations, on average.
*
...
/*  We feed the upper few bits of the key back into itself. This assures us that
 * the starting key affects the entire message. We go through some effort here
 * to prevent the key from exceeding 29 bits.  This is so the aritmetic
 * caluclation in a later statement, which impliments our autokey, won't
 * overflow, making the key go negative. Machine behavior in these cases does
 * not tend to be portable. */
 ...
 *       text, and salt.
 *
 *       I had to allow the older broken key calculation to let us decrypt old
 * files.  I hope to take this out eventually. **/
...

{
    ...
    /*  Yes, what I do here may look strange, but it gets the job done, and
     * portably at that.  */
