...
char *sfilnam;    /* save file name or NULL */
{
    ...
    if ( ( resolve_full_pathname(bfnam, bfnam) ) == FIOFNF ) { /* edit file
                                                                * found? */
...
VOID ttclose()
{
    fflush(stdout);
    fflush(stdin);
    ioctl(STDIN->_file, TCSETA, &old_in_termio);     /* restore terminal
                                                      * settings */
 ...
 * buffer.  Hurray for kludges!!!
 */
int typahead()
{
/* USG - SysV console i/o - From  "C Users Journal", April 1989, Vol. 7, # 9 */
    if ( !kbdqp ) {
        if ( fcntl( STDIN->_file, F_SETFL,
                    (kbdflgs | O_NDELAY) ) < 0 && kbdpoll )
            return (FALSE);
 ...
 * another directory such as :SL_TEMPS.  This diectory should have an ACL of
 *  +,RWE  which will allow all users to access it.  This diectory is where the
 * output file for this command is placed.  This was done because the sons of
 * MircoEMACS do not always have the same privileges.  The file is of the
 * pathname :TMP:pid.MICRO_EMACS_COMMAND where "pid" is the PID of the
 * MicroEMACS process.  Also note that this sort of follows the UNIX(tm)
 * convention for a common temporary file directory. The temp. file is created
 * with an ACL of  username,OWAR
 *
...
char *u_path, *a_path;
{
    ...
    /*
     *  step through the unix(tm) pathname and copy or translate bytes into the
     * temp. storage area.
     */
