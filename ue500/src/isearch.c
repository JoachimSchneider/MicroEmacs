 ...
 * (ITS).  Contains references to routines internal to SEARCH.C.
 *
 * REVISION HISTORY:
 *
 *  D. R. Banks 9-May-86
 ...
 *  - Made iterative search use search.c's scanner() routine. This allowed the
 * elimination of bakscan().
 *  - Put isearch constants into estruct.h
 *  - Eliminated the passing of 'status' to scanmore() and checknext(), since
 * there were no circumstances where it ever equalled FALSE. Dan Corkill
 * 6-Oct-87
 *      - Changed character loop to terminate with extended characters (thus
 * arrow keys, and most commands behave intuitively).
 *      - Changed META to be reread rather than simply aborting.
 *      - Conditionalized VMS alternates for ^S and ^Q to only apply to VMS
 * ports.  (Allowing ^X as a synonym for ^S defeats some of the benefits of the
 * first change above.)
 *
...
/*
 * Subroutine to do incremental reverse search.  It actually uses the same code
 * as the normal incremental search, as both can go both ways.
 */
...
/*
 * Subroutine to do an incremental search.  In general, this works similarly to
 * the older micro-emacs search function, except that the search happens as each
 ...
 * the end of the entire matched string.  Typing a Control-S will cause the next
 * occurrence of the string to be searched for (where the next occurrence does
 * NOT overlap the current occurrence).  A Control-R will change to a backwards
 * search, META will terminate the search and Control-G will abort the search.
 *  Rubout will back up to the previous match of the string, or if the starting
 * point is reached first, it will delete the last character from the search
 * string.
 *
 * While searching backward, each successive character will leave the cursor at
 * the beginning of the matched string.  Typing a Control-R will search backward
 * for the next occurrence of the string.  Control-S will revert the search to
 * the forward direction.  In general, the reverse incremental search is just
 * like the forward incremental search inverted.
 *
 * In all cases, if the search fails, the user will be feeped, and the search
 * will stall until the pattern string is edited back into something that exists
 * (or until the search is aborted).
 */
...

{
    ...
    register int (PASCAL NEAR *kfunc)();        /* ptr to the requested function
                                                 * to bind to */
    ...
    /*
     * Save the current search string and line position, in case we bounce out
...
start_over:                             /* This is a good place to start a
                                         * re-execution: */

    /*
     * Ask the user for the text of a pattern, and remember the column.
     */
...
/*              "ISearch: " */
    ...
    for (;; ) {                         /* ISearch per character loop */
        ...
        /*
         * Check for special characters first. That is, a control or ^X or FN or
         * mouse function. Most cases here change the search.
         */
        c = ectoc( expc = get_char() );

        if ( expc == isterm ) {         /* Want to quit searching?    */
        ...
        if ( expc == quotec )                           /* Quote character?
            ...
            if ( kfunc == forwsearch || kfunc == forwhunt ||kfunc == fisearch ||
                ...
                /*
                 * if cpos == 0 then we are either just starting or starting
                 * over.  Use the original pattern in pat, which has either not
                 * been changed or has just been restored. Find the length and
                 * re-echo the string.
                 */
                if ( cpos == 0 ) {
                ...
                /* Back up over the Rubout and the character it's rubbing out.
                 *  If it is a quoted char, rub the quote char out too.
                 */
                cmd_offset -= 2;
                if ( cmd_offset > 0 &&cmd_buff[cmd_offset - 1] == quotec )
                    cmd_offset--;
                cmd_buff[cmd_offset] = '\0';

                /*
                 * Reset everything - line and offset, search direction and
                 * pattern and jump tables, start the whole mess over
            ...
            /*
             * Presumably the key was a command key, yet strangely
             * uninteresting...
             */
        ...
        /*
         * I guess we got something to search for, so put the character in the
         * buffer and search for it.
         */
        pat[cpos++] = c;

        /*
         * Too many characters in the string?  Yup.  Complain about it, and
         * restore the old search string and its jump tables.
         */
        if ( cpos >= NPAT ) {
            mlwrite(TEXT166);
...
# endif
        ...
        if ( !status )          /* If we lost last time       */
            ...
            status = checknext(c, dir);                 /* See if still matches
                                                         * or find next */
    }
 ...
 * forward or backward.  If we can't find any more matches, "point"
 * is left where it was before.  If we do find a match, "point" will be at the
 * end of the matched string for forward searches and at the beginning of the
 * matched string for reverse searches.
 */
 ...
 * still true to whatever we're pointing to in the buffer. This routine will not
 * attempt to move the "point" if the match fails, although it will implicitly
 * move the "point" if we're forward searching, and find a match, since that's
 * the way forward isearch works.  If we are reverse searching we compare all
...
int dir;                                /* Search direction         */
{
    ...
    if ( dir == FORWARD ) {                     /* If searching forward     */
        ...
        patrn = (char *) pat;
        while ( *patrn ) {                      /* Loop for all characters in
                                                 * patrn   */
            if ( ( sts =
                       !boundry(curline, curoff, FORWARD) ) == FALSE ||
                 ( sts =
                       eq(nextch(&curline, &curoff, FORWARD),
                          *patrn) ) == FALSE )
    ...
    /*
     * If the 'next' character didn't fit in the pattern, let's go search for it
     * somewhere else.
     */
    if ( sts == FALSE )
        sts = scanmore(dir);
...
/*               "? command too long" */
    ...
    /*
     * If $isterm != meta character, then create the extended character as
     * getcmd() does.
     */
    if ( ( c = get_key() ) == isterm )
        return (isterm);
