...
#define MB_BUF  0x8000  /* static IDs should not look like they carry this flag
                         */
...
{
    { IDM_NULLPROC, nullproc }, { IDM_FILEFIND, filefind },
    { IDM_VIEWFILE, viewfile }, { IDM_INSFILE, insfile },
    { IDM_FILEREAD, fileread }, { IDM_FILENAME, filename },
    { IDM_FILESAVE, filesave }, { IDM_FILEWRITE, filewrite },
    { IDM_FILEAPP, fileapp }, { IDM_SETEKEY, setekey },
    { IDM_NEXTBUFFER, nextbuffer }, { IDM_USEBUFFER, usebuffer },
    { IDM_UNMARK, unmark }, { IDM_NAMEBUFFER, namebuffer },
    { IDM_KILLBUFFER, killbuffer }, { IDM_NARROW, narrow },
    { IDM_WIDEN, widen }, { IDM_LISTBUFFERS, listbuffers },
    { IDM_SPLITWIND, splitwind }, { IDM_DELWIND, delwind },
    { IDM_ONLYWIND, onlywind }, { IDM_NEXTWIND, nextwind },
    { IDM_PREVWIND, prevwind }, { IDM_MVUPWIND, mvupwind },
    { IDM_MVDNWIND, mvdnwind }, { IDM_NEXTUP, nextup },
    { IDM_NEXTDOWN, nextdown }, { IDM_ENLARGEWIND, enlargewind },
    { IDM_SHRINKWIND, shrinkwind }, { IDM_RESIZE, resize },
    { IDM_QUICKEXIT, quickexit }, { IDM_QUIT, quit },
    /* here just to compute the key binding: the actual IDM_QUIT selection is
     * processed as a special */
    { IDM_CUTREGION, cutregion }, { IDM_CLIPREGION, clipregion },
    { IDM_INSERTCLIP, insertclip }, { IDM_SETMARK, setmark },
    { IDM_REMMARK, remmark }, { IDM_SWAPMARK, swapmark }, { IDM_YANK, yank },
    { IDM_KILLREGION, killregion }, { IDM_COPYREGION, copyregion },
    { IDM_UPPERREGION, upperregion }, { IDM_LOWERREGION, lowerregion },
    { IDM_ENTAB, entab }, { IDM_DETAB, detab }, { IDM_TRIM, trim },
    { IDM_INDENT_REGION, indent_region }, { IDM_UNDENT_REGION, undent_region },
    { IDM_WORDCOUNT, wordcount }, { IDM_FILLPARA, fillpara },
    { IDM_KILLPARA, killpara }, { IDM_KILLTEXT, killtext },
    { IDM_OPENLINE, openline }, { IDM_DELFWORD, delfword },
    { IDM_DELBWORD, delbword }, { IDM_CAPWORD, capword },
    { IDM_LOWERWORD, lowerword }, { IDM_UPPERWORD, upperword },
    { IDM_DEBLANK, deblank }, { IDM_TWIDDLE, twiddle }, { IDM_TAB, tab },
    { IDM_QUOTE, quote }, { IDM_FORWSEARCH, forwsearch },
    { IDM_BACKSEARCH, backsearch }, { IDM_FORWHUNT, forwhunt },
    { IDM_BACKHUNT, backhunt }, { IDM_FISEARCH, fisearch },
    { IDM_RISEARCH, risearch }, { IDM_SREPLACE, sreplace },
    { IDM_QREPLACE, qreplace }, { IDM_GOTOMARK, gotomark },
    { IDM_GOTOLINE, gotoline }, { IDM_GETFENCE, getfence },
    { IDM_GOTOBOB, gotobob }, { IDM_GOTOEOB, gotoeob },
    { IDM_FORWPAGE, forwpage }, { IDM_BACKPAGE, backpage },
    { IDM_GOTOEOP, gotoeop }, { IDM_GOTOBOP, gotobop },
    { IDM_FORWLINE, forwline }, { IDM_BACKLINE, backline },
    { IDM_GOTOBOL, gotobol }, { IDM_GOTOEOL, gotoeol },
    { IDM_FORWWORD, forwword }, { IDM_BACKWORD, backword },
    { IDM_ENDWORD, endword }, { IDM_EXECPRG, execprg },
    { IDM_SPAWNCLI, spawncli }, { IDM_SPAWN, spawn }, { IDM_PIPECMD, pipecmd },
    { IDM_FILTER, f_filter }, { IDM_CTLXE, ctlxe }, { IDM_CTLXLP, ctlxlp },
    { IDM_CTLXRP, ctlxrp }, { IDM_NAMEDCMD, namedcmd },
    { IDM_EXECCMD, execcmd }, { IDM_EXECPROC, execproc },
    { IDM_EXECBUF, execbuf }, { IDM_EXECFILE, execfile },
    { IDM_BINDTOKEY, bindtokey }, { IDM_MACROTOKEY, macrotokey },
    { IDM_BINDTOMENU, bindtomenu }, { IDM_MACROTOMENU, macrotomenu },
    { IDM_UNBINDKEY, unbindkey }, { IDM_UNBINDMENU, unbindmenu },
    { IDM_DESKEY, deskey }, { IDM_SETVAR, setvar }, { IDM_DISPVAR, dispvar },
    { IDM_DESVARS, desvars }, { IDM_SHOWCPOS, showcpos }, { IDM_CTRLG, ctrlg },
    { IDM_FIND_SCREEN, find_screen }, { IDM_RENAME_SCREEN, rename_screen },
    { IDM_NEWSIZE, newsize }, { IDM_NEWWIDTH, newwidth },
    { IDM_DESBIND, desbind }, { IDM_DESFUNC, desfunc }, { IDM_APRO, apro },
    { 0, NULL }
};
...
typedef struct {    /* current menu description */
    ...
    int cm_x;           /* index of the last parent handle, -1 if entry located
                         * in main menu bar */
...
/* This function is called by the edit loop in main.c, when a MENU extended
...
/* ============                             */
int EXPORT FAR PASCAL  AboutDlgProc (HWND   hDlg,
                                     UINT   wMsg,
                                     WPARAM wParam,
                                     LPARAM lParam)
{
...
/* must be invoked through DialogBoxParam, with LOWORD(dwInitParam) set to TRUE
 * for global modes and FALSE for current buffer modes */

int EXPORT FAR PASCAL  ModeDlgProc (HWND   hDlg,
                                    UINT   wMsg,
                                    WPARAM wParam,
                                    LPARAM lParam)
{
...
/* if the entry is a menu item (not a popup), its ID is returned. If the entry
 * is a popup, the ID of the first entry in this popup is returned, added to
 * n*IDM_POPUP where n is the number of popup levels.
 *
 *  If the entry is neither an item nor a popup, an item id of 0 is returned */
...
KEYTAB * FAR PASCAL FindKeyBinding (void *Func)
{
    ...
    for ( KTp = &keytab[0]; KTp->k_type != BINDNUL; ++KTp ) {
        if ( ( (KTp->k_type == BINDFNC) && (KTp->k_ptr.fp == Func) ) ||
            ...
            if ( !(KTp->k_code & MOUS) ) break; /* found it! (we skip mouse
                                                 * bindings) */
    }
...
/* Returns the length (excluding the terminating NULL) of the text if it fits in
 * the supplied buffer, 0 otherwise (in which case the buffer contents are
 * unuseable). */

/* This function uses the ctrlx_key and meta_key static variables to avoid
 * scanning the key binding table too often. If these are NULL, however, it
 * looks them up. Therefore, these variables can be zeroed if there is a doubt
...
void FAR PASCAL InitMenuPopup (HMENU hMenu, LPARAM lParam)
{
    ...
    ctrlx_key = meta_key = 0;       /* these may have changed since the last
                                     * call to GetKeyText */
    ...
    for ( Position =  0; Position < ItemCount; Position++ ) {
        ...
        /* since we assume the items' order follows the binding table, we try to
         * optimize the lookup by starting from the last found entry */
        if ( EntryID >= IDM_POPUP ) {
            /* for popup menus MTp yields the nullproc entry */
            MTp = &MenuStaticBind[0];
        } else if ( EntryID >= IDM_DYNAMIC ) {
            /* for dynamic items, MTp is directly set to the proper entry in the
             * MenuDynBind */
        ...
        if ( MTp != &MenuStaticBind[0] ) UpdateMenuItemText (hMenu,
                                                             Position,
                                                             MTp);
        ...
        if ( notquiescent ) {
            ...
            /*-if quiescent, gray the invalid items, depending on appropriate
             * conditions */
            Enable = TRUE;
            switch ( EntryID ) {
            case IDM_FILESAVE:
...
static void PASCAL  SimulateExtendedKey (int ec)
{
    ...
    if ( in_room(5) ) {
        ...
        if ( prefix != 0 ) {
            ...
            in_put (prefix);
            if ( prefix & (MOUS >> 8) ) {
                /* in case the key is a mouse action, supply dummy mouse
                 * position info */
                in_put (1);
                in_put (1);
            }
...
BOOL FAR PASCAL MenuCommand (WPARAM wParam, LPARAM lParam)
{
    ...
    switch ( LOWORD(wParam) ) {
    /* the menu choices from here down to the default statement are valid even
     * in the not-quiescent case */

    case IDM_ABOUT:
        ProcInstance =
            MakeProcInstance ( (FARPROC)AboutDlgProc, hEmacsInstance );
    ...
    case IDM_CTRLG:
        /* this is a special case: we feed the abort key into the input stream
         */
...
InvokeHelp: WinHelp(hFrameWnd,
                    MainHelpFile,
                    HelpContext ? HELP_CONTEXT : HELP_INDEX,
                    HelpContext);
#endif
        MainHelpUsed = TRUE;
        break;
    ...
    default:
        ...
        case IDM_MODE:
            ProcInstance = MakeProcInstance ( (FARPROC)ModeDlgProc,
                                              hEmacsInstance );
            DialogBoxParam ( hEmacsInstance, "MODES", hFrameWnd, ProcInstance,
...
#if WINXP
            sp =
                (SCREEN_T*)GetWindowLongPtr( (HWND)( SendMessage(hMDIClientWnd,
                                                                 WM_MDIGETACTIVE,
                                                                 0, 0) ),
                                             GWL_SCRPTR );
#else
            sp =
                (SCREEN_T*)GetWindowLong( (HWND)( SendMessage (hMDIClientWnd,
                                                               WM_MDIGETACTIVE,
                                                               0,
                                                               0L) ),
                                          GWL_SCRPTR );
...
#if WINXP
        if ( GetWindowLongPtr( (HWND)SendMessage(hMDIClientWnd, WM_MDIGETACTIVE,
                                                 0, 0),
                               GWL_STYLE ) & WS_MAXIMIZE ) {
#elif WINDOW_MSWIN32
        if ( GetWindowLong( (HWND)SendMessage(hMDIClientWnd, WM_MDIGETACTIVE, 0,
                                              0L), GWL_STYLE ) & WS_MAXIMIZE ) {
#else
        if ( HIWORD( SendMessage (hMDIClientWnd, WM_MDIGETACTIVE, 0, 0L) ) ) {
#endif
            /* an MDI child is maximized ==> we have MDI buttons in the menu bar
             */
            Count -= 2;
        }
    }
...
/* Puts the text part of the menu name in Title (at most MAXMENUTITLE characters
 * including the \0) and the position number in *Posp. If no position is
...
/* The returned BOOL is TRUE if a matching menu entry was found, FALSE if no
 * such entry was found and FAILD if a syntax problem was diagnosed. The CURMENU
 * structure is updated if the entry was found.
 */
{
    HMENU hMenu;
    int ItemCount;
...
MenuLocated: CM->cm_pos = Pos - PosOffset;
...
static BOOL PASCAL  AddMenuEntry (char    *Name,
                                  UINT    ID,
                                  CURMENU *CM,
                                  WORD    *MenuType)
 ...
 * indicates that the menu bar has been modified */
{
    ...
    if ( Name != NULL ) {     /* our task is to set up a popup menu */
        ...
        if ( strcmp (EntryName, "-") == 0 ) {    /* only a separator */
            ...
            *MenuType |= MT_DUMMY;
            ID = 0;     /* contrary to SDK documentation, InsertMenu does not
                         * ignore the ID for a separator in 3.0! */
        } else {
            MenuFlags = MF_STRING;
        }
    }
...
/* called by bindtomenu and macrotomenu, once the function or macro name has
 * been parsed */
{
    BOOL Result;
    ...
    while ( ( Result = LocateMenu (Name, &CM) ) == TRUE ) {
        /* ...looping until we hit a piece of the menu path that does not yet
         * exist */
        HMENU hMenu;
    ...
    if ( ( Result =
               AddMenuEntry (Name, DynIndex + IDM_DYNAMIC, &CM,
                             &MenuType) ) != TRUE ) {
        return Result;
    }
    CurrentMenu = CM;
    ...
    if ( !IsMenuSeparator (hMenu, Pos) ) {
        if ( ( ID = GetMenuItemID (hMenu, Pos) ) != -1 ) {
            ...
            if ( ID == IDM_SCREEN_POPUP ) {
                /* this menu is used by the system's MDI manager to list the MDI
                 * childs. We cannot remove it */
                return FALSE;
            }
    ...
    while ( ( Result = LocateMenu (Name, &CM) ) == TRUE ) {
        ...
        if ( Name != NULL ) {     /* the menu path continues beyond this... */
            ...
            if ( (Pos = MenuEntryCount (hMenu) - 1) < CM.cm_pos ) {
                /* we just deleted the item at the menu's end. This position is
                 * no longer valid */
                CM.cm_pos = Pos;
            }
            CurrentMenu = CM;
