...
#if     OS2NPM
/*
 * Os2def.h defines COLOR, but no MSC stuff needs it. We need COLOR as defined
 * in estruct.h, so edit it out of os2def.h.
 */
# include        <conio.h>
...
/*
 * To minimize the amount of buffer that VioShowBuf has to update, we keep track
 * of the lowest and highest bytes in the logical video buffer which have been
 * modified.
 */
...
PASCAL NEAR os2putc(int c)
{
    ...
    if ( c == '\b' ) {                          /* backspace        */
        ...
        cell = (0x00ff & c) |
               (revflag ? *(USHORT *)&os2rcell : *(USHORT *)&os2cell);
        i = os2row * term.t_ncol + os2col;
...
# endif
    ...
    VioSetCurPos(initial.vioModeInfo.row - 1, initial.vioModeInfo.col - 1, 0);
}
