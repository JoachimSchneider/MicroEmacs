 ...
 *        as a (0/CTRF/32) in the input stream to the kernel.
 *      - stock() in bind.c now interprets ^@ as <control><space>
 *        and getecnam() returns it as ^@ so we can see it.
 ...
 *      7 Nov 2021 Joachim Schneider
 *      - Reimplemanted fillpara code: It is now in trParFill:
 *        o It is undoable: The old code caused memory overwrites when followed
 *           by an undo.
