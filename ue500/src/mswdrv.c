...
int EXPORT FAR PASCAL  MLHistDlgProc (
    HWND   hDlg,
    UINT   wMsg,
    WPARAM wParam,
    LPARAM lParam)
...
/* This function is needed because the message line only shows the last message.
 * If a macro fails, it usually generates a slew of errors (each beginning by a
 * \n). Eventually, calling this function in the core editor loop will pop up a
 * dialog box showing up to MAXMLHIST-1 past messages */
...
PASCAL mlhistory (void)
{
    ...
    PushMLHist ();
    mlferase ();
    ProcInstance = MakeProcInstance ( (FARPROC)MLHistDlgProc, hEmacsInstance );
...
int c;      /* character to write (or 0 for dummy write) */
{
    ...
    /*-update the cursor's position and the UpdateCol range. Update the message
     * line buffer if that's where the fun is */
    if ( c != 0 ) {
        if ( CurrentCol < UpdateCol.leftmost ) UpdateCol.leftmost = CurrentCol;
        if ( CurrentCol >
             UpdateCol.rightmost ) UpdateCol.rightmost = CurrentCol;
...
static int PASCAL mswflush ()
{
    ...
    /* some caret movement is directly handled by the driver (see mswputc) */
    return 0;
} /* mswflush */
...
{
    /*-perform IO windows switching depending on the addressed row: if beyond
     * the IO screen, the message line is addressed*/
...
static int PASCAL msweeop ()
{
    ...
    if ( CurrentRow + 1 < IOScr->s_nrow ) {
        ...
        InvalidateCells (hIOWnd, 0, CurrentRow + 1, IOScr->s_ncol,
                         IOScr->s_nrow);
    }
    mswflush ();
...
{
    /* nothing to do: the description of reverse video fields is kept up to date
     * in the physical screen by the display.c code */
    return 0;
} /* mswrev */
...
char    *res;
{
    return TRUE;   /* $SRES is read-only in MSWIN, but returning an error may
...
/* called by screen.c after the screen structure has been allocated. The size of
 * the screen will be determined by the window's size. returns TRUE if
 * successful. */
{
    MDICREATESTRUCT cs;
    BOOL Maximized;
...
#if WINXP
    Maximized =
        (GetWindowLongPtr( (HWND)SendMessage(hMDIClientWnd, WM_MDIGETACTIVE, 0,
                                             0),
                           GWL_STYLE ) & WS_MAXIMIZE);
#elif WINDOW_MSWIN32
    Maximized =
        (GetWindowLong( (HWND)SendMessage (hMDIClientWnd, WM_MDIGETACTIVE, 0,
                                           0L),
                        GWL_STYLE ) & WS_MAXIMIZE);
#else
 ...
 * mswdisp.c */
{
    HWND hScrWnd;
    BOOL Maximized;
    int RetryCount = -1;        /* prevents infinite loop if frame window can't
                                 * be resized properly */
    BOOL FrameIconic;
    ...
    {
        /*-If this is iconized, we have to restore it first to be allowed to
         * change its size. If it is the active MDI child and is maximized, we
         * will actually resize the frame window so, if the later is maximized,
         * we restore it first. */
#if WINXP
        HWND hActiveScr;
        hActiveScr = (HWND)SendMessage(hMDIClientWnd, WM_MDIGETACTIVE, 0, 0);
        Maximized = (hScrWnd == hActiveScr) &&
                    (GetWindowLongPtr(hScrWnd, GWL_STYLE) & WS_MAXIMIZE);
#elif WINDOW_MSWIN32
        HWND hActiveScr;
    ...
    do {/*-Now, proceed with resizing... */
        ...
        /*-process change of width: we move the right edge. If it would go off
         * bounds, we move the left edge back to compensate, but never below 0
         */
        Delta = ClientSize.x - OldClientRect.right;
        if ( (TargetRect.right + Delta) > BoundingRect.right ) {
            TargetRect.left += (BoundingRect.right - TargetRect.right)- Delta;
        ...
        /*-process change of height: we move the bottom edge. If it would go off
         * screen, we move the top edge up to compensate, but never beyond 0 */
        Delta = ClientSize.y - OldClientRect.bottom;
        if ( (TargetRect.bottom + Delta) > BoundingRect.bottom ) {
            TargetRect.top += (BoundingRect.bottom - TargetRect.bottom)- Delta;
        ...
        if ( Maximized && FrameIconic && Win31API ) {
            ...
            SetWindowPos (Maximized ? hFrameWnd : hScrWnd,
                          0,
                          TargetRect.left,
                          TargetRect.top,
                          TargetRect.right - TargetRect.left,
    ...
    } while (Maximized);    /* if Maximized, we loop once in case a change of
                             * menu bar height has prevented us from getting the
...
/* called by screen.c when selecting a screen for current */
{
    if ( !InternalRequest ) {
        InternalRequest = TRUE;
        SendMessage (hMDIClientWnd, WM_MDIACTIVATE, (WPARAM)sp->s_drvhandle,
                     0L);
#if WINXP
        ...
        if ( HIWORD( SendMessage (hMDIClientWnd, WM_MDIGETACTIVE, 0, 0L) ) ) {
             ...
             * InternalRequest, the new size has not yet been processed by
             * emacs, so we have to fix that */
            newwidth ( TRUE,
                       DisplayableColumns (sp->s_drvhandle, 0, &EmacsCM) );
