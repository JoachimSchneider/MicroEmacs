...
/*      0x22,*/ 'd',    'D',    CTRF|'D',
/*      0x23,*/ 'f',    'F',    CTRF|'F',
/*      0x24,*/ 'g',    'G',    CTRF|'G',/*     0x25,*/ 'h',    'H',    CTRF|'H',
/*      0x26,*/ 'j',    'J',    CTRF|'J',
/*      0x27,*/ 'k',    'K',    CTRF|'K',
/*      0x28,*/ 'l',    'L',    CTRF|'L',
/*      0x29,*/ ';',    ':',    0,
...
/*      0x3f,*/ '9',    SPEC|'Z',       0,
/*      0x40,*/ ' ',    SHFT|' ',       0,
/*      0x41,*/ CTRF|'H',       SHFT|'D',       0,
/*      0x42,*/ CTRF|'I',       SHFT|'I',       0,
/*      0x43,*/ CTRF|'M', CTRF|'M', CTRF|'M',
/*      0x44,*/ CTRF|'M', CTRF|'M', CTRF|'M',
/*      0x45,*/ CTRF|'[',       0,      0,
/*      0x46,*/ SPEC|'D',       0,      0,
/*      0x47,*/ 0,      0,      0,
/*      0x48,*/ 0,      0,      0,
...
/*      0x4a,*/ '-',    0,      0,
/*      0x4b,*/ 0,      0,      0,
/*      0x4c,*/ SPEC|'P',       SHFT|SPEC|'P',  CTRF|SPEC|'P',
/*      0x4d,*/ SPEC|'N',       SHFT|SPEC|'N',  CTRF|SPEC|'N',
/*      0x4e,*/ SPEC|'F',       SHFT|SPEC|'F',  CTRF|SPEC|'F',
/*      0x4f,*/ SPEC|'B',       SHFT|SPEC|'B',  CTRF|SPEC|'B',
/*      0x50,*/ SPEC|'1',       SHFT|SPEC|'1',  CTRF|SPEC|'1',
/*      0x51,*/ SPEC|'2',       SHFT|SPEC|'2',  CTRF|SPEC|'2',
/*      0x52,*/ SPEC|'3',       SHFT|SPEC|'3',  CTRF|SPEC|'3',
/*      0x53,*/ SPEC|'4',       SHFT|SPEC|'4',  CTRF|SPEC|'4',
/*      0x54,*/ SPEC|'5',       SHFT|SPEC|'5',  CTRF|SPEC|'5',
/*      0x55,*/ SPEC|'6',       SHFT|SPEC|'6',  CTRF|SPEC|'6',
/*      0x56,*/ SPEC|'7',       SHFT|SPEC|'7',  CTRF|SPEC|'7',
/*      0x57,*/ SPEC|'8',       SHFT|SPEC|'8',  CTRF|SPEC|'8',
/*      0x58,*/ SPEC|'9',       SHFT|SPEC|'9',  CTRF|SPEC|'9',
/*      0x59,*/ SPEC|'0',       SHFT|SPEC|'0',  CTRF|SPEC|'0',
/*      0x5a,*/ '(',    0,      0,
/*      0x5b,*/ ')',    0,      0,
/*      0x5c,*/ '/',    0,      0,
/*      0x5d,*/ '*',    0,      0,
...
/*
 * This function is called once to set up the terminal device streams. On VMS,
 * it translates TT until it finds the terminal, then assigns a channel to it
...
# endif
    ...
    new_win.Flags = WINDOWCLOSE | SMART_REFRESH | ACTIVATE |WINDOWDRAG |
                    WINDOWDEPTH | WINDOWSIZING | SIZEBRIGHT |RMBTRAP |
                    NOCAREREFRESH;
...
doevent()
{
    ...
    while ( event = GetMsg(win->UserPort) ) {
        ...
        if ( class == CLOSEWINDOW ) {
            quit(FALSE, 0);
            stuffibuf(255, 0, 0);               /* fake a char to force quit to
                                                 * work */
        }
...
/*                      "[Cannot write filter file]" */
    ...
    newcli = Open("CON:0/0/640/200/MicroEMACS Subprocess", NEW);
    xstrcat(line, " <ram:fltinp >ram:fltout");
    Execute(line, 0L, newcli);
    s = TRUE;
    Close(newcli);
    sgarbf = TRUE;
...
/*  Big flame..... AZTEC C documents, but seems to be missing the standard
 * library realloc() function.  A quick replacement is below.  Note that this
 * ALWAYS copies to a new block, and is thus inherently less efficient than a
 * native one would be.
 */
