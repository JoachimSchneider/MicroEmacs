...
shell(void)
{
    ...
    /*
     * We are actually setting up a shell inside a shell here. Is there a better
     * way?
     */
    return ( system(shell) );
}
...
execprog(char *cmd)
{
     ...
     */
    return ( 0 == DosExecPgm(failName, NFILEN, EXEC_SYNC, args, 0, results,
                             prog) );
}
