...
*   extcode()
*
*   This routine is common to the MSDOS and OS/2 implementations. It is used to
* resolve extended character codes from the keyboard into EMACS printable
* character specifications.
*
*   This implementation can handle the extended AT-style keyboard if one is
* fitted.
*
*   I don't know what happens on an XT but I suspect that it should work as if
* there were no extended keys.
*
...
/*
 *  This implementation distinguishes between the cursor controls on the number
 * pad and those on the grey keypad if an AT-style keyboard is fitted.
 */
 ...
 *
 *  Adds a new escape sequence to the sequence table. I am not going to try to
 * explain this table to you in detail. However, in short, it creates a tree
 * which can easily be transversed to see if input is in a sequence which can be
 * translated to a function key (arrows and find/select/do etc. are treated like
 * function keys).  If the sequence is ambiguous or duplicated, it is silently
 * ignored.
 *
