/*  VMS.C   Operating system specific I/O and spawning functions For VAX/VMS
 * operating system for MicroEMACS 4.00 Copyright 1995 by Jeffrey A. Lomicka and
 * Daniel M. Lawrence
 *
 *       All-new code replaces the previous VMS/SMG implementation which
 * prevented using non-SMG terminal drivers (ansi, termcap).  New approach to
 * terminal I/O, new (and more) subprocess control functions, Kept emacs
 * support, mail/notes interface.
 *
 *       Some of the above may still be wishlist.
 *
 *       12-Dec-89  Kevin A. Mitchell Start work on RMSIO code.
 */
#include        <stdio.h>
#include        "estruct.h"
#if     VMS
...
# include ctype
/*
 *       These are the LIB$SPAWN mode flags.  There's no .h for them in VAX C
 * V2.4.
 */
...
/*
 *       This routine returns a pointer to a descriptor of the supplied string.
 * The descriptors are static allocated, and up to
 *       "NUM_DESCRIPTORS" may be used at once.  After that, the old ones are
 * re-used. Be careful!
 *
 ...
 * descptr( s)  String descriptor for asciz buffer s DESCPTR( s)    String
 * descriptor for buffer s, using sizeof()
 */
...
typedef struct {       /* More terminal characteristics (hidden in the status
                        * block) */
    short status;
    char txspeed;
    char rxspeed;
...
typedef struct {       /* Status block for ordinary terminal reads */
    unsigned short status, len, term, tlen;
} TTIOSB;

typedef struct {       /* Status block for mailbox reads */
    unsigned short status, len;
    long sender_pid;
} MBIOSB;

typedef struct {       /* Messages from the terminal or TW driver */
...
/*
 *       If we come from ME$EDIT, the "suspend-emacs" is not allowed, since it
 * will tend to wake itself up and re-hiberneate itself, which is a problem.
 */
static short called = 0;                /* TRUE if called from ME$EDIT */
/*
 *       short_time[ 0] is the negative number of 100ns units of time to wait.
 *  -10000 is 1ms, therefore 200ms (2 tenths of a second) is
 *       -2,000,000.  Hopefully this is long enough for the network delay that
 * might be involved between seeing the ESC and seeing the characters that
 * follow it.
 *
...
{       /* Data arrived from the terminal */
    waiting = 1;
    if ( (ttiosb.status == SS$_TIMEOUT) || (ttiosb.status & 1) ) { /* Read
                                                                    * completed
                                                                    * okay,
                                                                    * process
                                                                    * the data
                                                                    */
        if ( ttiosb.len ) { /* Got some data, adjust input queue parameters */
            tylen += ttiosb.len;
            tyin += ttiosb.len;
            test(SYS$WAKE(0, 0) );
            next_read(1);
...
static next_read(int flag)
{
    ...
    if ( waiting || stalled ) {
        ...
        if ( size >= MINREAD ) { /* Only read if there is enough room */
            test(SYS$QIO(
                     0, vms_iochan,
                     flag ?  IO$_READVBLK | IO$M_NOECHO | IO$M_TRMNOECHO |
                     IO$M_NOFILTR |
                     IO$M_TIMED:       IO$_READVBLK | IO$M_NOECHO |
                     IO$M_TRMNOECHO |
                     IO$M_NOFILTR, &ttiosb, readast, 0, &tybuf[ tyin],
                     flag ? size : 1, 0, noterm, 0, 0) );
            stalled = 0;
        } else stalled = 1;
    }
}
...
/*
 * The argument msgbuf points to the buffer we want to insert our broadcast
 * message into.
 */
static brdaddline(BUFFER *msgbuf)
{
    ...
    /*
     * If one of the windows has the buffer displayed, flag that window for
     * update.
     */
    wp = wheadp;
    while ( wp != NULL ) {
...
static mbreadast()
{
    if ( mbiosb.status & 1 ) { /* Read completed okay, check for hangup message
                                */
        if ( mbmsg.msgtype == MSG$_TRMHANGUP ) {
            /* Got a termination message, process it */
        } else if ( mbmsg.msgtype == MSG$_TRMUNSOLIC ) { /* Got unsolicited
                                                          * input, get it */
            next_read(1);
        } else if ( mbmsg.msgtype == MSG$_TRMBRDCST ) { /* Got broadcast, get it
                                                         */
                                                        /* Hard-coding the
                                                         * mbmsg.brdcnt to 511
                                                         * is a temp solution.*/
        ...
        test(SYS$QIO(                   /* Post a new read to the associated
                                         * mailbox */
                 0, mbchan, IO$_READVBLK, &mbiosb, mbreadast, 0, &mbmsg,
                 sizeof ( mbmsg), 0, 0, 0, 0) );
...
PASCAL NEAR ttopen()
{
    ...
    status = LIB$ASN_WTH_MBX(   /* Create a new PY/TW pair */
        ...
        &vms_iochan,
        &mbchan);
    if ( (status & 1) == 0 ) { /* The assign channel failed, was it because of
                                * the mailbox? */
        if ( status == SS$_DEVACTIVE ) { /* We've been called from NOTES, so we
                                          * can't use the mailbox */
    ...
    if ( mbchan ) test(SYS$QIO(          /* Post a read to the associated
                                          * mailbox */
                           0, mbchan, IO$_READVBLK, &mbiosb, mbreadast, 0,
                           &mbmsg, sizeof ( mbmsg), 0, 0, 0, 0) );
/*
 *       Fetch the characteristics and adjust ourself for proper operation.
 */
    test(SYS$QIOW(
             0, vms_iochan, IO$_SENSEMODE, &orgttiosb, 0, 0, &orgchar,
             sizeof ( orgchar), 0, 0, 0, 0) );
    newchar = orgchar;
...
/*
 *       Hostsync allows super-fast typing (workstation paste, PC send-file)
 * without loss of data, as long as terminal supports XON/XOFF.  VWS and
 * DECWindows terminal emulators require HOSTSYNC for PASTE operations to work,
 * even though there is no wire involved.
 */
    newchar.tt1 |= TT$M_HOSTSYNC;
/*
 *       If you MUST, and if you know you don't need XON/XOFF synchronization,
 * you can get ^S and ^Q back as data by defining XONDATA in ESTRUCT.H.  This is
 * guarnteed to fail on VT125, VT100's over 3600 baud, any serial line terminal
 * with smooth scroll enabled, VT200's over 4800 baud.  This is guarnteed to
 * WORK if you are using a VT330/340 with SSU enabled, a VWS or DECWindows
 * terminal emulator.  Note that if XONDATA is not set, I trust the settings the
 * user has, so you just $ SET TERM /[NO]TTSYNC as you wish.
 */
# if XONDATA
    newchar.tt1 &= ~TT$M_TTSYNC;
# endif
/*
 *       I checked in DISPLAY.C, and verified that the mrow and mcol numbers
 * aren't used until after ttopen() is called.  I override the terminal-supplied
 * numbers with large numbers of my own, so that workstation terminal resizes
 * will work to reasonable limits.
 *
 *       I don't just use the current sizes as the maximum, becuase it's
 * possible to resize the terminal emulator after Emacs is started, or even to
 * disconnect and reconnect with a new terminal size, so the maximums must not
 * change over multiple calls to ttopen().
 *
 *       Also note that I do the changes to newchar, so that the actual terminal
 * window will be reduced to the maximum values Microemacs will support.
 */
 ...
 */
    test(SYS$QIOW(
             0, vms_iochan, IO$_SETMODE, 0, 0, 0, &newchar, sizeof ( newchar),
             0, 0, 0, 0) );
/*
 *       For some unknown reason, if I don't post this read (which will likely
 * return right away) then I don't get started properly. It has something to do
 * with priming the unsolicited input system.
 */
    test(SYS$QIO(
             0, vms_iochan,
             IO$_READVBLK | IO$M_NOECHO | IO$M_TRMNOECHO |IO$M_NOFILTR |
             IO$M_TIMED,
             &ttiosb, readast, 0, tybuf, sizeof ( tybuf), 0, noterm, 0, 0) );
/*
 *       Initialize the short_time value for esc-reads.  Larger values may be
 * needed on network links.  I'm still experimeinting to get the best numbers.
 */
...
PASCAL NEAR ttclose()
{
    if ( tolen > 0 ) { /* Buffer not empty, flush out last stuff */
        test(SYS$QIOW(0, vms_iochan, IO$_WRITEVBLK  | IO$M_NOFORMAT, 0, 0, 0,
                      tobuf, tolen, 0, 0, 0, 0) );
        tolen = 0;
    }
    test(SYS$CANCEL(vms_iochan) );      /* Cancel any pending read */
    test(SYS$QIOW(
             0, vms_iochan, IO$_SETMODE, 0, 0, 0, &orgchar, sizeof ( orgchar),
             0, 0, 0, 0) );
...
{
    tobuf[ tolen++] = c;
    if ( tolen >= sizeof ( tobuf) ) { /* Buffer is full, send it out */
        test(SYS$QIOW(0, vms_iochan, IO$_WRITEVBLK | IO$M_NOFORMAT, 0, 0, 0,
                      tobuf, tolen, 0, 0, 0, 0) );
        tolen = 0;
    }
...
{
/*
 *       I choose to ignore any flush requests if there is typeahead pending.
 *  Speeds DECNet network operation by leaps and bounds (literally).
 */
    if ( tylen == 0 ) if ( tolen != 0 ) { /* No typeahead, send it out */
            test(SYS$QIOW(0, vms_iochan, IO$_WRITEVBLK  | IO$M_NOFORMAT, 0, 0,
                          0, tobuf, tolen, 0, 0, 0, 0) );
            tolen = 0;
        }
}
/*
 *       grabnowait is a routine that tries to read another character, and if
 * one doesn't come in as fast as we expect function keys sequences to arrive,
 * we return -1.  This is called after receving ESC to check for next character.
 *  It's okay to wait too long, but the length of the delay controls how log the
 * user waits after hitting ESC before results are seen.
 *
 *       Note that we also wake from hibernation if a character arrives, so this
 * never causes an undue delay if the user it actually typing.
 */
int PASCAL NEAR grabnowait()
{
    if ( tylen == 0 ) { /* Nothing immediately available, hibernate for a short
                         * time */
...
int PASCAL NEAR ttgetc()
{
    ...
    chkbrdcst();
    while ( tylen == 0 ) { /* Nothing to send, wait for something interesting */
        ttflush();
        test( SYS$HIBER() );
        chkbrdcst();
    }
...
int PASCAL NEAR pipecmd()
{
    ...
    test(LIB$SPAWN(descptr(line), DESCPTR("NL:"), descptr(filnam), 0, 0, 0, 0,
                   0, 0, 0, 0) );
    TTopen();
    TTflush();
    sgarbf = TRUE;
...
/*                      "[Cannot write filter file]" */
    ...
    test(LIB$SPAWN(descptr(line), descptr(filnam1), descptr(filnam2), 0, 0, 0,
                   &s, 0, 0, 0, 0) );
    TTopen();
    TTflush();
    sgarbf = TRUE;
...
/*
 * Do a wild card directory search (for file name completion) fspec is the
 * pattern to match.
 */
char *PASCAL NEAR getffile(char *fspec)
{
     ...
     * isolate the filename and extension and append that on to the original
     * path
     */
...
char *PASCAL NEAR getnfile()
{
     ...
     * isolate the original path, isolate the filename and extension, and append
     * filename/extension on to the original path
     */
...
/*
 *       The following ME$EDIT entry point is used when MicroEmacs is called up
 * from MAIL or NOTES.  Note that it may be called more than once, and that
 ...
 */
    ...
    test(LIB$DELETE_LOGICAL(
             DESCPTR("MICROEMACS$PARENT"), DESCPTR("LNM$JOB") ) );
    test(LIB$GETJPI(&JPI$_PID, 0, 0, &pid, 0, 0) );
    test(LIB$SET_LOGICAL(
             DESCPTR("MICROEMACS$PROCESS"), descptr(int_asc(pid) ),
             DESCPTR("LNM$JOB") ) );
    pid = asc_int(env);
    test(SYS$WAKE(&pid, 0) );

    for (;; ) { /* Hibernate until MICROEMACS$COMMAND is defined */
        test(SYS$HIBER() );
    ...
    test(LIB$DELETE_LOGICAL(
             DESCPTR("MICROEMACS$COMMAND"), DESCPTR("LNM$JOB") ) );

    TTopen();

    argv[ 0] = env;
    argc = 1;
    for ( ; ; ) { /* Define each argument */
        if ( *env == 0x80 ) { /* Seperator */
            argv[argc++] = env+1;
            if ( argc > 15 ) break;
            *env++ = 0;
        } else if ( *env++ == 0 ) break;
 ...
 */
    test(LIB$SET_LOGICAL(
             DESCPTR("SYS$DISK"), descptr(argv[ 0]), 0) );
/*
...
# if RMSIO
/*
 * Here are the much faster I/O routines.  Skip the C stuff, use the VMS I/O
 * calls.  Puts the files in standard VMS format, too.
 */
...
PASCAL NEAR ffropen(char *fn)
{
    ...
    if ( status==RMS$_FLK ) {
         ...
         * Add the SHRPUT option, allowing shareability with other writers. This
         * lets us read batch logs and stuff like that. I don't turn it on
...

{
    ...
    if ( fline == NULL )
        if ( ( fline = malloc(flen = fab.fab$w_mrs?fab.fab$w_mrs+1:32768) ) ==
             NULL )
            return (FIOMEM);
                                ...
                                int                   *pargcapacity)
{
    ...
    if ( *pargc == *pargcapacity )
        *pargv =
            realloc( *pargv,
                     sizeof (**pargv) * (*pargcapacity += ADDSPEC_INCREMENT) );
...
VOID PASCAL NEAR expandargs(int *pargc, char ***pargv)
{
    ...
    struct dsc$descriptor result_filespec=
    {
    ...
    while ( argc-- ) {
        ...
        {
            strlen(*argv), DSC$K_DTYPE_T, DSC$K_CLASS_S, *argv
        };
        ...
        if ( **argv != '-' &&
             ( strchr(*argv,
                      '%') || strchr(*argv, '*') ||strstr(*argv, "...") ) ) {
            /* search for all matching filenames */
            while ( ( LIB$FIND_FILE(&filespec, &result_filespec,
                                    &context) ) & 1 ) {
                int i;
                ...
                for ( i=0; i<result_filespec.dsc$w_length; i++ )
                    if ( is_upper(result_filespec.dsc$a_pointer[i]) )
                        result_filespec.dsc$a_pointer[i] = tolower(
                            result_filespec.dsc$a_pointer[i]);
