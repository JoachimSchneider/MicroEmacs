...
/* Data segment switching macroes. The wS argument MUST be a stack variable */
...
#  endif
    ...
    /*-Scan segment list, attempting to find one where suballocation is possible
     */
    Seg = &SegList;
    for (;; ) {
        if ( Seg->next == NULL ) {
            ...
            SWITCH_DS(wSeg)
            LockData (0);   /* this ensures the segment's selector will never
                             * change */
            /* note that allocating a GMEM_FIXED segment would have been cleaner
             * but, in Windows 3.0, such segments also end up being page-locked
             */
            RESTORE_DS Seg->next = (SEGHEADER*)MAKELONG(SEGHEADOFFSET, wSeg);
            Seg = Seg->next;
            Seg->next = NULL;
            Seg->alloc_count = 0;
            NewSeg = TRUE;
...
#  endif
        ...
        if ( Seg == OptimumSeg ) continue;  /* skip this already tried one */
...
#  endif
        ...
        if ( (Block != NULL) || NewSeg ) {
            OptimumSeg = Seg;   /* next malloc will try this segment first */
...
#  endif
    LocalFree (hBlock);
    RESTORE_DS Seg = (SEGHEADER *)( MAKELONG( SEGHEADOFFSET, HIWORD(block) ) );
#  if MEMTRACE
...
#  endif
    if ( --(Seg->alloc_count) == 0 ) {
        /* this segment is no longer used!
         *  Let's get rid of it */
        SEGHEADER   *sp;
...
#  if MEMTRACE
    ...
    mtr[mtrx].m_ptr.m_block = hBlock ? Block : NULL;
    mtr[mtrx].m_cnt =
        ( (SEGHEADER *)( MAKELONG(SEGHEADOFFSET, wSeg) ) )->alloc_count;
    mtr[mtrx].m_size = RealSize;
    mtr[mtrx++].m_optimseg = OptimumSeg;
#  endif
    ...
    if ( hBlock ) return Block;
        ...
        /* we have to malloc, possibly into another segment, and copy the data
         * over */
