...
#if BSD || FREEBSD || LINUX || USG || AIX || AUX || SMOS || HPUX8 || HPUX9 || \
    SUN || XENIX || (AVVION || TERMIOS)
...
# endif /* BSD || FREEBSD || LINUX || SUN || HPUX8 || HPUX9 || (AVVION ||
         * TERMIOS) */
...
#  define DIRENTRY        direct
# endif /* XENIX */
# if ( (USG || AIX || AUX ) && !VAT) || SMOS || HPUX8 || HPUX9 || SUN || \
    (AVVION || TERMIOS)
#  include <dirent.h>                   /* Directory entry definitions  */
#  define DIRENTRY        dirent
# endif /* ((USG || AIX || AUX) && !VAT) || SMOS || HPUX8 || HPUX9 || SUN ||
         * (AVVION || TERMIOS) */
...
# endif /* USG || AIX || AUX || SMOS || HPUX8 || HPUX9 || (SUN && !TERMIOS) ||
         * XENIX */
# if (AVVION || TERMIOS)
...
# if (BSD && !TERMIOS)
    ...
    if ( ioctl(0, TIOCGETP, &oldsgtty) ||
         ioctl(0, TIOCGETC, &oldtchars) ||ioctl(0, TIOCGLTC, &oldlchars) )
        return (-1);
    ...
    /* Set tty modes */
    if ( ioctl(0, TIOCSETP,
               &cursgtty) ||ioctl(0, TIOCSETC, blank) ||ioctl(0, TIOCSLTC,
                                                              blank) )
        return (-1);
...
#  endif /* SMOS */
    ...
    /* Set new modes */
    /* I do not believe the flow control settings of the OS should be diddled by
     * an application program. But if you do, change this 1 to a 0, but be
     * warned, all sorts of terminals will get grief with this */
#  if     1
...
# if (BSD && !TERMIOS)
    if ( ioctl(0, TIOCSETP, &oldsgtty) ||
         ioctl(0, TIOCSETC, &oldtchars) ||ioctl(0, TIOCSLTC, &oldlchars) )
        return (-1);
...
# endif /* (BSD && !TERMIOS) */
# if USG || AIX || AUX || SMOS || HPUX8 || HPUX9 || (SUN && !TERMIOS) || \
    XENIX || (AVVION || TERMIOS)
    unsigned char ch;
...
# endif /* (BSD && !TERMIOS) */
# if USG || AIX || AUX || SMOS || HPUX8 || HPUX9 || SUN || XENIX || (AVVION || \
                                                                     TERMIOS)
    int count;
    unsigned char ch;
...
# if USG || AIX || AUX || SMOS || HPUX8 || HPUX9 || XENIX || (AVVION || TERMIOS)
        sh = "/bin/sh";
# endif /* USG || AIX || AUX || SMOS || HPUX8 || HPUX9 || XENIX || (AVVION ||
         * TERMIOS) */
...
#endif /* BSD || FREEBSD || LINUX || USG || AIX || AUX || SMOS || HPUX8 || HPUX9
        * || SUN || XENIX || (AVVION || TERMIOS) */
