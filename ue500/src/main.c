...
static int PASCAL NEAR getbasekey P0_()
{
    ...
    notquiescent = -1;  /* will be <= 0 only if get_key() is called directly
                         ...
                         * windows-specific actions (menus, sizing, etc...) when
                         * not called from the lowest level of the editor */
    c = get_key();
    notquiescent = 1;
...
/*          "\n--- Press any key to Continue ---" */
    ...
    /* Notify user of any intercepted system messages. VMS only right now.
     */
#if     VMS
    if ( pending_msg ) {
        pending_msg = FALSE;
    ...
    if ( c == reptc ) {   /* ^U, start argument   */
        ...
        while ( ((c = GETBASEKEY()) >= '0' && c <= '9') || c == reptc || c == '-' ) {
            ...
            /*
             * If first digit entered, replace previous argument with digit and
             * set sign.  Otherwise, append to arg.
             */
            else {
                if ( !mflag ) {
                    n = 0;
        ...
        /*
         * Make arguments preceded by a minus sign negative and change the
         * special argument "^U -" to an effective "^U -1".
         */
        if ( mflag == -1 ) {
            if ( n == 0 )
                n++;
                            ...
                            int, n /* prefix value */)
{
     ...
     */
    if ( c == ' ' && (curwp->w_bufp->b_mode & MDWRAP) && fillcol > 0 &&n >= 0 &&
